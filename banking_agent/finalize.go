...
func cmdFinalize(args []string) {
	...
	controlNamePrefix := "reconcile-spending-banking_agent_auto_adjust"
	if state.FileType == "copay" {
		controlNamePrefix = "reconcile-spending-banking_agent_copay_auto_adjust"
	}
	controlFilename := fmt.Sprintf("%s-%s.json", controlNamePrefix, uuidv4.String())
	...
	// SQL — batch IN delete on ref_id
	const tableName = "bill_payment_transaction"
	const batchSize = 500
	sqlPath := filepath.Join(outputDir, "insert_banking_agent_transactions.sql")
	deleteSqlPath := filepath.Join(outputDir, "delete_banking_agent_transactions.sql")
	...
	fmt.Printf("  ✓ %s\n", controlFilename)
	fmt.Printf("  ✓ insert_banking_agent_transactions.sql  (%d rows)\n", len(sqlRowsVals))
	fmt.Printf("  ✓ delete_banking_agent_transactions.sql\n")
