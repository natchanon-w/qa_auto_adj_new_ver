...
func cmdFinalize(args []string) {
	...
	controlFilename := fmt.Sprintf("reconcile-lending-disburse_to_itmx_billers-%s.json", uuidv4.String())
	...
	controlData := map[string]interface{}{
		"project":             "reconcile",
		"dataset":             "VFS-DCB",
		"table":               "disburse_to_itmx_billers",
	...
	const batchSize = 500
	sqlPath := filepath.Join(outputDir, "insert_disburse_to_itmx_billers.sql")
	deleteSqlPath := filepath.Join(outputDir, "delete_disburse_to_itmx_billers.sql")
	...
	fmt.Printf("  ✓ %s\n", controlFilename)
	fmt.Printf("  ✓ insert_disburse_to_itmx_billers.sql  (%d rows)\n", len(sqlRowsVals))
	fmt.Printf("  ✓ delete_disburse_to_itmx_billers.sql\n")
