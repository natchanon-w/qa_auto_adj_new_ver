...
const sourceTable = "cardless_deposit_transactions"
...
func finalizeOne(workDir string) {
	...
	if refColIdx < 0 {
		fmt.Println("Column 'transaction_reference_id' not found in CSV header")
		os.Exit(1)
	}
	...
	// Control JSON — matches real production reconcile-spending control file format
	// (dataset "spending", table "cardless_auto_adjust", sha256, compression "PGP"),
	// not the stale "VFS-DCB"/md5/no-compression format still emitted by the
	// processor repo's own local-scripts/generate_test_data.go.
	uuidv4, _ := uuid.NewRandom()
	controlFilename := fmt.Sprintf("reconcile-spending-cardless_auto_adjust-%s.json", uuidv4.String())
	...
	// SQL — insert new source rows, update existing rows back to PROCESSING, delete by req_ref_id
	const batchSize = 500
	sqlPath := filepath.Join(outputDir, "insert_cardless_deposit_transactions.sql")
	updateSqlPath := filepath.Join(outputDir, "update_cardless_deposit_transactions.sql")
	deleteSqlPath := filepath.Join(outputDir, "delete_cardless_deposit_transactions.sql")
	insertHeader := buildInsertHeader(sourceTable)
	...
	fUpd, _ := os.Create(updateSqlPath)
	for i := 0; i < len(quotedRefs); i += batchSize {
		end := i + batchSize
		if end > len(quotedRefs) {
			end = len(quotedRefs)
		}
		fUpd.WriteString(fmt.Sprintf("UPDATE \"public\".\"%s\" SET \"transaction_status\" = 'PROCESSING' WHERE \"req_ref_id\" IN (%s);\n", sourceTable, strings.Join(quotedRefs[i:end], ", ")))
	}
	fUpd.Close()
	...
	for i := 0; i < len(quotedRefs); i += batchSize {
		...
		fDel.WriteString(fmt.Sprintf("DELETE FROM \"public\".\"%s\" WHERE \"req_ref_id\" IN (%s);\n", sourceTable, strings.Join(quotedRefs[i:end], ", ")))
	}
	fDel.Close()
	...
	fmt.Printf("  ✓ %s\n", controlFilename)
	fmt.Printf("  ✓ insert_cardless_deposit_transactions.sql  (%d rows)\n", len(sqlRowsVals))
	fmt.Printf("  ✓ update_cardless_deposit_transactions.sql\n")
	fmt.Printf("  ✓ delete_cardless_deposit_transactions.sql\n")
